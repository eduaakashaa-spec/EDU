...
| Database models | ✅ Done | `models.py` — User, DasaLead, Prediction, Payment, PageLead, ContactInquiry, Announcement, ScheduleEvent, AlumniProfile, MentorMeeting, MentorMessage, CollegeSurvey (+ `models_membership.py`: MembershipApplication, MembershipInvoice, DocSequence) |
...
| **EA Admin Control Panel** | ✅ Done | `/admin` — a full dashboard: grouped KPI bands (members, membership & ₹ revenue, leads/inquiries/surveys, College-Guide activity & ₹ payouts, weekly growth) + **4 Chart.js charts** (8-week activity line, members-by-type doughnut, applications-by-status bar, top-lead-sources bar) + recent-activity feeds. Plus member management (tier / validity / password / add / delete with lockout guards), announcements + schedule, per-member message drafts, and **Email Templates** (see below). Grouped dropdown nav. All KPI blocks run as single conditional-aggregate SELECTs (`app/services/queries.py` `count_if`/`sum_if`) — `/admin` is 23 queries/load, not 47; extend the aggregate rather than adding `.count()` calls. `routes/admin_portal.py`, `Announcement` + `ScheduleEvent` models |
| **Admin Email Templates** | ✅ Done | `/admin/templates` — 19 scenario templates grouped by category (Members & Onboarding · Admissions & Payments · College Guides · Engagement & Season), incl. onboarding-assessment invites (student/parent), session confirmed/recap, report delivered, documents, application approved, payment reminder/received, JoSAA/TNEA deadline alerts, win-back, feedback, inquiry response. The **To** field is a searchable dropdown over all known contacts (members/guides/leads/inquiries, deduped) that autofills name/college placeholders. Live logo-branded preview; draft opens **from `eduaakashaa@gmail.com`** (Gmail compose `authuser=…` / mailto / copy-formatted-HTML). Nothing is auto-sent. Registry: `app/services/email_templates.py`; composer: `templates/admin/templates.html`; recipient directory: `admin_portal._recipient_directory()` |
| **College Guides** (peer network) | ✅ Done | Branded **"College Guides"** (internal tier is still `mentor`). Public `/alumni-network` signup creates a `mentor`-tier login + `AlumniProfile`. **Resume is an uploaded file** (PDF/DOC/DOCX, extension+magic-validated, ≤5 MB, stored privately in **Cloudflare R2** — the object key lives on the profile; admins read it via a short-lived presigned link. Legacy rows may still hold a pasted share link — `admin_resume` handles both); **photo** validated by type+magic, capped ~200 KB. Per-IP rate limit + email dedup; referral links. **Guide portal `/mentor`**: **₹ earnings** — a guide just answers a parent's questions and earns **₹500–1000** (referral bonus **₹1000**), sessions table, referral tracking, guide↔admin messages, editable profile. Admin `/admin/alumni`: resume-link/photo, status, log sessions + ₹ payouts (mark paid), reply, referral tree. Resume storage: `services/r2.py` (boto3 S3 client → R2; server-side upload, private bucket, presigned GET; configured via `R2_ENDPOINT`/`R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY`/`R2_BUCKET`). `MentorMeeting.kind` keeps meeting/video/referral/bonus/adjustment for internal tracking. Models: `AlumniProfile`, `MentorMeeting`, `MentorMessage`; `routes/alumni.py` |
| **Invoices (standalone GST)** | ✅ Done | `routes/invoices.py` — admin **Finance ▾** tab. Free-form line items (desc/qty/rate) + discount + additional fee + GST (rate per invoice, CGST+SGST vs IGST split by customer state vs home state, computed **on top**/exclusive via `pricing.compute_invoice_totals`). Admin creates (`/admin/invoices/new`, dynamic JS rows + live total), views a printable invoice (`invoice_view.html`, browser Print→PDF), **emails it** to the customer (inline HTML + tokenised link `/invoice/<token>`, no login), records payment (mode + ref + status). Money in paise; the **seller details are snapshotted** onto each invoice so later edits never mutate issued invoices. Transaction-safe numbering shares the `DocSequence` `invoice` series (`EA/NNNN`). **Editable seller profile** (`Setting` table + `services/company.py`): GSTIN/PAN/bank/etc. set once at `/admin/invoice-settings`, edited any time, falling back to `config.COMPANY` defaults. No server-side PDF lib (deliberate). Emails need SMTP. Models: `Invoice`, `Setting`. |
| **Member profile + Document Verification** | ✅ Done | `/profile` (`routes/members.py`) — replaces the old `/dashboard` (which now 302s there). Every logged-in non-guide user gets a profile: account info, announcements/events, their **counselling sessions + notes** (`MemberMeeting`, logged by admin, read-only to the member), and **document verification**. Doc flow (`DocVerification`, one row per requested doc, cyclic): admin **Document Verify** tab (`/admin/documents`) lists premium members → per-member page requests documents from a curated `DOC_TYPES` list (or custom) → emails the member → member uploads each file (validated, stored privately in **R2**) → **Submit** notifies `DOC_NOTIFY_EMAIL` (default `eduaakashaa@gmail.com`) → admin **approve / reject-with-comment** emails the member each cycle → when all Approved, member gets an "all verified" email. Statuses: Requested→Submitted→Approved/Rejected. All automated emails need SMTP configured (else silent no-op). Nav: header "Dashboard" → "My Profile"; admin People ▾ → "Document Verify". |
| **Membership Onboarding Assessment** | ✅ Done | `/onboarding-assessment` — a two-part, dyadic profiling instrument (student 60 items + parent 34 items: RIASEC, Big Five, SDT motivation, subject self-efficacy, work values, light aptitude, ALIGN pairs) delivered as a mobile **one-question-per-screen stepper** (progress bar; widgets for likert / most-least forced-choice / rank / pick-2 / choice-pair / num / SJT / open). Answers save to `onboarding_responses` keyed by item id; student & parent halves pair on `family_key`. Admin **People → Onboarding Assessments** lists families and, per submission, renders both halves by module+tag with a **Copy-for-AI** bundle that points at the rubric + dossier template in `ONBOARDING_ASSESSMENT.md`; confidential items (S50/P34) are counsellor-only. Definition-driven: the whole question set is one list in `services/onboarding_assessment.py` (drives form + validation + admin display). `routes/onboarding.py`, `OnboardingResponse` model. Design spec: `ONBOARDING_ASSESSMENT.md` |
| **College Experience Survey** | ✅ Done | Free, public, deliberately exhaustive `/college-survey` (10 sections, ~96 questions: academics, placements, infra, hostel/food, amenities, campus life & safety, admin, location, verdict). Definition-driven — the whole question set is one `SECTIONS` list in `routes/survey.py` (renders form + validates POST + drives admin display); answers stored in a single `CollegeSurvey.responses_json` blob keyed by question id, with a few first-class columns for search/sort. Per-IP rate limit. Soft funnel into the College Guides network. Admin `/admin/surveys` — responses **grouped by college** (collapsible) — + per-response detail. Linked from the **More** nav dropdown. |
...
- **Secrets:** `.env`, `mcp.json`, `.claude/` are gitignored — never commit. Bank/PAN/GST/
  admin creds, `DATABASE_URL` and the `R2_*` keys live only in `.env` / Render env vars.
