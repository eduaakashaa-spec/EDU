...
| **EA Admin Control Panel** | ✅ Done | `/admin` — a full dashboard: grouped KPI bands (members, membership & ₹ revenue, leads/inquiries/surveys, College-Guide activity & ₹ payouts, weekly growth) + **4 Chart.js charts** (8-week activity line, members-by-type doughnut, applications-by-status bar, top-lead-sources bar) + recent-activity feeds. Plus member management (tier / validity / password / add / delete with lockout guards), announcements + schedule, per-member message drafts, and **Email Templates** (see below). Grouped dropdown nav. `routes/admin_portal.py`, `Announcement` + `ScheduleEvent` models |
| **Admin Email Templates** | ✅ Done | `/admin/templates` — pick a template (welcome ·new user/premium/College Guide, thank-you for a guide session, thank-you for a survey, payment credited, guide matched, renewal, application received), fill placeholders, and open a ready draft **from `eduaakashaa@gmail.com`** via a Gmail compose link (`authuser=…`) or mailto — plus a branded HTML preview and copy-formatted-email. Nothing is auto-sent. `EMAIL_TEMPLATES` in `routes/admin_portal.py`, client-side composer in `templates/admin/templates.html` |
