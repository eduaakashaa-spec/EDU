...
| **Alumni / Mentor Network** | ✅ Done | Public `/alumni-network` signup creates a **`mentor`-tier login** + `AlumniProfile`. **Resume is a pasted share link** (Drive/Dropbox/URL, http(s)-validated) — kept out of Postgres; **photo** is validated by type+magic and capped ~200 KB, stored in Postgres. Per-IP rate limit + email dedup; referral links. **Mentor portal `/mentor`**: **₹ earnings/calls/payouts** (referral bonus **₹2000**), sessions table, referral tracking, mentor↔admin messages, editable profile — all scoped to the mentor. Admin `/admin/alumni`: resume-link/photo, status, log sessions + **₹ payouts** (mark paid), reply to messages, referral tree. Models: `AlumniProfile`, `MentorMeeting`, `MentorMessage`; `routes/alumni.py` |
| **College Experience Survey** | ✅ Done | Free, public, deliberately exhaustive `/college-survey` (10 sections, ~96 questions: academics, placements, infra, hostel/food, amenities, campus life & safety, admin, location, verdict). Definition-driven — the whole question set is one `SECTIONS` list in `routes/survey.py` (renders form + validates POST + drives admin display); answers stored in a single `CollegeSurvey.responses_json` blob keyed by question id, with a few first-class columns for search/sort. Per-IP rate limit. Soft funnel into the mentor network. Admin `/admin/surveys` list + per-response detail. Linked from the **More** nav dropdown. |
