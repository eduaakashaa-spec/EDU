...
| **College Guides** (peer network) | ✅ Done | Branded **"College Guides"** (internal tier is still `mentor`). Public `/alumni-network` signup creates a `mentor`-tier login + `AlumniProfile`. **Resume is a pasted share link** (http(s)-validated, kept out of Postgres); **photo** validated by type+magic, capped ~200 KB. Per-IP rate limit + email dedup; referral links. **Guide portal `/mentor`**: **₹ earnings** — a guide just answers a parent's questions and earns **₹500–1000** (referral bonus **₹1000**), sessions table, referral tracking, guide↔admin messages, editable profile. Admin `/admin/alumni`: resume-link/photo, status, log sessions + ₹ payouts (mark paid), reply, referral tree. `MentorMeeting.kind` keeps meeting/video/referral/bonus/adjustment for internal tracking. Models: `AlumniProfile`, `MentorMeeting`, `MentorMessage`; `routes/alumni.py` |
